...
(function() {
	...
	g
			.exend({
				...
				gtbyn : function(n, v, w) {
					var a, co = (w || window).document.getElementsByName(n);
					if (!v)
						return co;
					else {
	...
	g.exend({
		...
		i8 : (j.match(/msie (\d+)/) || [])[1] == 8 && !/opera/.test(j),
		ch : /chrome/.test(j),
		op : /opera/.test(j),
